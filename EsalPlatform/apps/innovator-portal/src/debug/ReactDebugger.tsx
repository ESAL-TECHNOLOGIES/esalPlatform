...
export const useStateDebug = function <T>(
  initialState: T,
  ...
  if (!debugContext) {
    console.warn(
      "⚠️ useStateDebug called outside ReactDebugProvider, falling back to regular useState"
    );
...
export const useEffectDebug = function (
  ...
  if (!debugContext) {
    console.warn(
      "⚠️ useEffectDebug called outside ReactDebugProvider, falling back to regular useEffect"
    );
...
export const ReactDebugProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
