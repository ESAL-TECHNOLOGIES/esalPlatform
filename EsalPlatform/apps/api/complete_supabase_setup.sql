...
-- This will create all necessary tables, policies, and functions for the ESAL Platform

-- ========================================
-- STEP 1: CREATE TABLES
-- ========================================

-- 1. Ideas Table (with AI-generated fields)
...
VALUES ('idea-files', 'idea-files', false, 3145728, NULL)
...
-- ========================================
-- STEP 11: ADD MISSING COLUMNS (IF NEEDED)
-- ========================================

-- Add AI-related columns to ideas table if they don't exist
DO $$ 
BEGIN
    -- Add ai_generated column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'ideas' AND column_name = 'ai_generated') THEN
        ALTER TABLE ideas ADD COLUMN ai_generated BOOLEAN DEFAULT FALSE;
    END IF;
    
    -- Add problem column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'ideas' AND column_name = 'problem') THEN
        ALTER TABLE ideas ADD COLUMN problem TEXT;
    END IF;
    
    -- Add solution column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'ideas' AND column_name = 'solution') THEN
        ALTER TABLE ideas ADD COLUMN solution TEXT;
    END IF;
    
    -- Add target_market column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'ideas' AND column_name = 'target_market') THEN
        ALTER TABLE ideas ADD COLUMN target_market TEXT;
    END IF;
    
    -- Add ai_metadata column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'ideas' AND column_name = 'ai_metadata') THEN
        ALTER TABLE ideas ADD COLUMN ai_metadata JSONB;
    END IF;
    
    -- Add ai_score column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'ideas' AND column_name = 'ai_score') THEN
        ALTER TABLE ideas ADD COLUMN ai_score DECIMAL(3,2);
    END IF;
END $$;
...
WHERE id IN ('idea-files', 'avatars');

-- Success message
SELECT 
    '🎉 COMPLETE DATABASE SETUP FINISHED!' as status,
    'Tables: ideas, files, profiles' as tables_created,
    'Buckets: idea-files, avatars' as storage_created,
    'RLS: Enabled with security policies' as security,
    'Next: Run your debug script!' as next_step;
